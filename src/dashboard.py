...
def render_dashboard(cleaned, analysis_date, context_key, dashboard=None, actions=None, verification=None):
    ...
    with st.sidebar:
        ...
        threshold = st.number_input(
            ...
            help="An open deal becomes stalled on this exact day. Changes update all results immediately.",
        )
        st.caption("Analysis date: {} · Amounts use your file's currency.".format(analysis_date))
    with verification:
    ...
    with st.sidebar:
        ...
        selections = {}
        for field in (f for f in FILTER_FIELDS if f in assessed):
