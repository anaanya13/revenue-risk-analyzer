...
Run `bash scripts/check.sh` from the project folder. The complete suite currently has 79 tests. New tests cover hand-calculated results, thresholds 1/3/30/60/61/3650, date changes, empty and closed-only selections, zero values, absent follow-ups, decimal amounts, sample data, filtered populations, query-like labels, deliberate mismatches and UI failure paths.
